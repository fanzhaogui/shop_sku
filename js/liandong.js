...
$(function () {
    ...
    var step = {
        ...
        Creat_Table: function () {
            ...
            var bCheck = true;//是否全选   // 217/4/7日 这里应该不是判断为全选，而是只要有选中了的值
            var columnIndex = 0;
            $.each(SKUObj, function (i, item){
                arrayColumn.push(columnIndex);
                columnIndex++;
                arrayTile.push(SKUObj.find("li").eq(i).html().replace("：", ""));// 规格名称
                var itemName = "Father_Item" + i;
                //选中的CHeckBox取值
                var order = new Array();
                $("." + itemName + " input[type=checkbox]:checked").each(function (){ // 规格下的规格项
                    order.push($(this).val());
                });
